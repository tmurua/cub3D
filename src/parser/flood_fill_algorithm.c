...
/*   Updated: 2025/04/01 19:58:26 by tmurua           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	perform_flood_fill(t_game *game)
{
	...
	result = flood_fill_on_copy(map_copy, game->map.rows, cols, &start);
...
int	flood_fill_on_copy(char **map_copy, int rows, int cols, t_map *start)
{
	t_game	temp;

	temp.map.lines = map_copy;
	temp.map.rows = rows;
	temp.map.cols = cols;
...
{
	if (r < 0 || c < 0 || r >= game->map.rows || c >= game->map.cols)
		return (-1);
	if (game->map.lines[r][c] == '1' || game->map.lines[r][c] == 'V')
		return (0);
	if (game->map.lines[r][c] == ' ')
		return (-1);
	game->map.lines[r][c] = 'V';
